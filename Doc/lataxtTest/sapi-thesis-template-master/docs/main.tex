\documentclass[12pt, twosides]{report}

\include{preamble}
\usepackage{hyperref}

\title{
	{Elektronikai alkatrész teszter}\\
	{\large Sapientia\\
	Erdélyi Magyar Tudományegyetem, Marosvásárhely}
}
\author{
	Lukács, Botond\\
	\texttt{lukacs.botond@student.ms.sapientia.ro}
	\and
	dr. Túrós, László-Zsolt\\
	\texttt{turosl@ms.sapientia.ro }	
}
\date{2022}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}

\includepdf[pages={1}]{pdfs/allamvizsga_borito_RO.pdf}
\includepdf[pages={1}]{pdfs/elolap_Lukacs_Botond.pdf}
\includepdf[pages={1,2,3}]{pdfs/LukacsBotond.pdf}
\section*{Extras}
\input{chapters/abstractRo}
\pagebreak

\includepdf[pages={1}]{pdfs/allamvizsga_borito_HU.pdf}

\section*{Kivonat}
\input{chapters/abstractHu}
\pagebreak

\section*{Abstract}
\input{chapters/abstractEn}
\pagebreak

\pagenumbering{gobble}

\tableofcontents

\listoffigures

\chapter{Bevezető}
\pagenumbering{arabic}
\input{chapters/bevezeto}

\chapter{Elméleti megalapozás és szakirodalni tanulmány}
\input{chapters/elmeletiMegalapozas}

\chapter{Rendszer specifikációja és architektúrája}
\input{chapters/rendszerSpecifikacio}

\chapter{Részletes tervezés}
\input{chapters/reszletesTervezes}

\chapter{Üzembe helyezés és kísérleti eredmények}
\input{chapters/uzembeHelyezes}

%következtetés
\chapter{Következtetések}
\input{chapters/kovetkezetetesek}

%irodalomjegyzek
\addcontentsline{toc}{chapter}{Irodalomjegyzék}

\bibliographystyle{ieeetr}
\bibliography{References}

%appendix
%\appendix
%\chapter{Függelék}
%\input{chapters/fuggelek}
\includepdf[pages={1}]{pdfs/Utolap.pdf}


\end{document}
